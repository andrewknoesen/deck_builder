# deck_builder — Memory (phase history archive)

This file holds the full per-phase record — Why/Ground truth/Design/Interview outcome/Concrete
steps/Shipped writeups, and everything each phase's own follow-ups turned up — for every phase
that's been shipped, parked, or otherwise settled. It exists purely to keep
[`PLAN.md`](PLAN.md) readable: `PLAN.md` now holds just the current summary (its `## Status`
section) and what's still actionable (`## Deferred`); the detailed "how we got here" for each
phase lives here instead.

**This is a pure relocation, not a summary.** Every phase body below is unchanged from what
previously lived in `PLAN.md` — moved, not rewritten or compressed. `PLAN.md`'s own Status section
has always refused to summarize its own history (an inaccurate summary is just another thing that
can go stale or overclaim); the same reasoning applies here, so nothing below has been
paraphrased.

**How to use this file**: start at `PLAN.md`'s `## Status` section for the current picture of
what's shipped/in-progress/deferred. Come here when you need the actual reasoning behind a past
decision, a phase's exact interview outcome, or what a "Shipped" writeup says really happened
(including deviations from the original plan) — the phase headers below are in the same order
`PLAN.md` used to have them in.

Moved here 2026-08-23, per product-owner request ("the plan is getting noisy") — `PLAN.md` had
grown to ~3,100 lines across eleven shipped/parked phases plus a Phase 0 archive block.
